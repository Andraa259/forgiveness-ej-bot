    ...
    for ind_name, items in data_aspek[aspek]:
        ...
        with st.container():
            ...
            for txt in items:
                ...
                # SESSION KEY UNIK: Menggunakan nama aspek dan index agar data tidak hilang saat pindah slide
                key_base = f"{aspek.replace(' ', '_')}_{global_idx}"
                ...
                # Tanpa perlu tekan Enter: Nilai tersimpan otomatis di session_state[key]
...
if st.session_state.step == 0:
    ...
    *   **Kesesuaian**: Kesesuaian aitem yang disusun dalam alat ukur sudah sesuai dengan indikatornya.
    ...
    if st.button("Mulai Penilaian 🚀"):
        if st.session_state.get("p_nama") == "" or st.session_state.get("p_kerja") == "":
    ...
    render_page_content(aspek_aktif)
    
    if st.session_state.step == 3:
        st.text_area("Catatan/Saran Keseluruhan (Akan diletakkan di bawah 'Catatan' tabel):", key="saran_akhir_global")
    ...
    with c2:
        if st.session_state.step < 3:
            ...
            if st.button("🚀 KIRIM HASIL"):
                nama_p = st.session_state.get("p_nama", "Expert")
                kerja_p = st.session_state.get("p_kerja", "-")
                
                with st.spinner("Memproses Word..."):
                    doc = Document("Form Validasi Expert Judgement Ayinn Ver. 3.docx")
                    # Identitas
                    for p in doc.paragraphs:
                        if "Nama\t\t:" in p.text: p.text = f"Nama\t\t: {nama_p}"
                        if "Pekerjaan\t:" in p.text: p.text = f"Pekerjaan\t: {kerja_p}"
                    
                    table = doc.tables[0]
                    for row in table.rows:
                        aitem_doc = row.cells[2].text.strip()
                        # MAPPING GLOBAL: Mencari kecocokan di seluruh data_aspek slide 1, 2, dan 3
                        found = False
                        for asp, indicators in data_aspek.items():
                            g_idx = 0
                            for ind_name, items in indicators:
                                for txt_ori in items:
                                    key_prefix = f"{asp.replace(' ', '_')}_{g_idx}"
                                    # Matching 25 karakter awal agar aman dari typo kecil/spasi di Word
                                    if txt_ori[:25] in aitem_doc:
                                        row.cells[3].text = str(st.session_state.get(f"{key_prefix}_kj", 4))
                                        row.cells[4].text = str(st.session_state.get(f"{key_prefix}_rel", 4))
                                        row.cells[5].text = str(st.session_state.get(f"{key_prefix}_kes", 4))
                                        row.cells[6].text = str(st.session_state.get(f"{key_prefix}_ket", ""))
                                        found = True
                                        break
                                    g_idx += 1
                                if found: break
