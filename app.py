...
# Identitas Default (Sesuai Word Testing)
...
# --- DATA INDIKATOR (ACUAN UNTUK MASTER DATA) ---
data_aspek = {
    "Pemaafan Diri": [
...
# PRE-FILLED MASTER DATA (Sesuai Word Testing)
if 'master_data' not in st.session_state:
    # Pola: 1,1,1 (TestX1), 2,2,2 (TestX2), 3,3,3 (TestX3), 4,4,4 (TestX4), 1,1,1 (TestX5), 2,2,2 (TestX6)
    md = {}
    patterns = [1, 2, 3, 4, 1, 2] # Pola Skor
    # Khusus Slide 1 Item 4 di Word tertulis Test24, lainnya berurutan
    kets = {
        "Pemaafan Diri": ["Test11", "Test12", "Test13", "Test24", "Test15", "Test16", "Test21", "Test22", "Test23", "Test24", "Test25", "Test26"],
        "Pemaafan Orang Lain": ["Test31", "Test32", "Test33", "Test34", "Test35", "Test36", "Test41", "Test42", "Test43", "Test44", "Test45", "Test46"],
        "Pemaafan Situasi": ["Test51", "Test52", "Test53", "Test54", "Test55", "Test56", "Test61", "Test62", "Test63", "Test64", "Test65", "Test66"],
    }
    
    for slide_key, slide_items in data_aspek.items():
        all_txt = [item for sub in slide_items for item in sub[1]]
        for i, txt in enumerate(all_txt):
            skor = patterns[i % 6] if i < 6 else patterns[(i-6) % 6]
            if slide_key == "Pemaafan Diri" and i >= 6: # Slide 2
                skor = [3, 4, 1, 2, 3, 4][i-6]
            
            md[txt] = {
                "kj": skor, "rel": skor, "kes": skor, 
                "ket": kets[slide_key][i]
            }
...
if st.session_state.step == 0:
    ...
    errors = [t for t in current_page_items if st.session_state.master_data[t]["kj"] == 0]
    ...
    with nav2:
        ...
        if st.button(btn_label):
            if errors: st.error(f"⚠️ Mohon lengkapi skor (tidak boleh 0).")
